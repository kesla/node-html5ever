// Taken from https://raw.githubusercontent.com/jsdom/jsdom/master/test/to-port-to-wpts/misc.js
...
describe("browser/index", () => {
    specify("notfound_getelementsbyclassname", () => {
        const doc = new Html5EverDom().window.document;
        const { body } = doc;
    ...
    specify("basic_getelementsbyclassname", () => {
        ...
        body.appendChild(p);
        const elements = doc.getElementsByClassName("first-p");
        assert.equal(elements[0], p, "p and first-p");
    });
    ...
    specify("multiple_getelementsbyclassname", () => {
        ...
        body.appendChild(p);
        const first = doc.getElementsByClassName("first-p")[0];
        const second = doc.getElementsByClassName("second")[0];
        const third = doc.getElementsByClassName("third")[0];
    ...
    specify("testclassnameworksasexpected", () => {
        ...
        assert.equal(
            p.className,
            "first-p",
            "class attribute is same as className",
        );
        p.className += " second";
    ...
    specify("remove_nonexistantattribute", () => {
        ...
        assert.doesNotThrow(
            () => body.removeAttribute("non-existant"),
            "setValue_throws_NO_MODIFICATION_ERR",
        );
    });
    ...
    specify("render_singletag", () => {
        ...
        assert.equal(
            out.match(/<\/img>/),
            null,
            "end tag not included in output",
        );
    });
    ...
    //     assert.doesNotThrow(
    //         () => doc.write(html),
    //         "setValue_throws_INVALID_CHARACTER_ERR",
    //     );
    // });
    ...
    specify("parse_doctype_containing_newline", () => {
        ...
        const doc = new Html5EverDom(html).window.document;
    ...
    //     const doc = new Html5EverDom(`<html><head></head><body>
