// FROM https://github.com/jsdom/jsdom/blob/master/test/util.js
...
exports.readTestFixture = async (relativePath) => {
    ...
    //     try {
    //         const response = await self.fetch(getTestFixtureUrl(relativePath), {
    //             method: "GET",
    //             signal,
    //         });
    //         if (!response.ok) {
    //             throw new Error(
    //                 `Unexpected status ${response.status} fetching ${response.location}`,
    //             );
    //         }
    ...
    return fs.promises.readFile(path.resolve(__dirname, relativePath), {
        encoding: "utf8",
    });
};
...
//         t.ok(
//             true,
//             "test ignored; not running with the canvas npm package installed",
//         );
//         done();
//         return false;
//     }
...
//         const options = {
//             key: fs.readFileSync(
//                 path.resolve(__dirname, "api/fixtures/key.pem"),
//             ),
//             cert: fs.readFileSync(
//                 path.resolve(__dirname, "api/fixtures/cert.pem"),
//             ),
//         };
