// taken from https://github.com/jsdom/jsdom/blob/04f6c13f4a4d387c7fc979b8f62c6f68d8a0c639/test/to-port-to-wpts/parsing.js
...
describe("jsdom/parsing", () => {
    ...
    //         "<ul><template><div>Hello world!</div></template></ul>",
    //     ).window;
    ...
    specify("innerHTML of <script type='text/html'> (GH-575)", () => {
        const { document } = new Html5EverDom(
            "<script type='text/html'>script innerHTML</script>",
        ).window;
    ...
    specify("attributes containing '<' and '>' (GH-494)", () => {
        const { document } = new Html5EverDom(
            "<p title='<'>stuff</p><p title='>'>more</p><p>just testing</p>",
        ).window;
    ...
    specify("empty attributes (GH-488)", () => {
        const { document } = new Html5EverDom("<div ng-app></div>").window;
    ...
    specify("omitting optional closing tags (GH-482)", () => {
        const { document } = new Html5EverDom("<p>First<p>Second<p>Third")
            .window;
    ...
    specify("crazy attribute names (GH-368)", () => {
        const { document } = new Html5EverDom("<p <='' FAIL>stuff</p>").window;
    ...
    specify("attribute named 'constructor' (GH-625)", () => {
        const { document } = new Html5EverDom(
            "<element constructor='Hello'></element>",
        ).window;
    ...
    //     const { document } = new Html5EverDom("<element></element>").window;
    ...
    specify("HTML entities without ; (GH-821)", () => {
        const { document } = new Html5EverDom("<a>&#x61</a>").window;
    ...
    specify("Added < and > near <script> tags (GH-826)", () => {
        const { document } = new Html5EverDom("<<script>alert(1);//<</script>")
            .window;
    ...
    specify("Chinese characters as a 'tag name' (GH-719)", () => {
        const { window } = new Html5EverDom(`<div>chinese here:<中文></div>`);
        assert.strictEqual(
            window.document.body.innerHTML,
            "<div>chinese here:&lt;中文&gt;</div>",
        );
    });
    ...
    specify("Missing <html> tags (GH-555)", () => {
        ...
        const docA = new Html5EverDom(
            doctype + "<html><head><title></title></head><p>",
        ).window.document;
        const docB = new Html5EverDom(
            doctype + "<head><title></title></head><p>",
        ).window.document;
    ...
    specify("More missing <html> and <body> tag tests (GH-88)", () => {
        const doc1 = new Html5EverDom("<html><body></body></html>").window
            .document;
        assert.strictEqual(doc1.body.innerHTML, "");

        const doc2 = new Html5EverDom("<body></body>").window.document;
        assert.strictEqual(doc2.body.innerHTML, "");

        const doc3 = new Html5EverDom("gavin!").window.document;
    ...
    specify("pre tag with < and > characters (GH-755)", () => {
        const { document } = new Html5EverDom(
            "<pre>[task.h:277] - RunnableMethod<DOMStorageDispatcherHost,void " +
    ...
    specify("xmlns doesn't cause empty prefix", () => {
        ...
        const dom = new Html5EverDom(html);
