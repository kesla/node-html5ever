...
describe("jsdom/selectors", () => {
  specify("div:last-child > span[title] (GH-972)", () => {
    const { document } = (new Html5EverDom("<div><div><span title='title text'>text</span></div></div>")).window;
  ...
  specify("div[title=''] (GH-1163)", () => {
    const { document } = (new Html5EverDom(`<!doctype html><html><head></head><body>
  ...
  specify("querySelectorAll smoke test", () => {
    const { document } = (new Html5EverDom(`<html><body><div id="main"><p>Foo</p><p>Bar</p></div><div id="next">` +
